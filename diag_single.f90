subroutine single_diag_arpack(nestate,dim,method,particle,renorm_flag,initial_vector,verbosesw,ev,evector)
...
10       continue
    ...
    !if (ido.ne.99) then
            ...
            ! c
      !print*,workd(ipntr(1))
            call avs (workd(ipntr(1)), workd(ipntr(2)), method,particle)
       ...
       !rvec = 1
            ...
            if ( ierr .ne. 0) then
               ...
               do j=1, nconv
                  ...
                  ! c               %---------------------------%
                  ! c
                  call avs(v(1,j), ax, method,particle)
...
!!$         deallocate(v)
         return
       end subroutine single_diag_arpack
       ...
       subroutine avs (v, w, method,particle)
         ...
         integer :: i,j,method,particle
         integer :: dim

         w=0.D0
         !print*,"test1"
         If(particle.eq.1) then !!!!particle=1 corresponding to the photon system !!!!!!!!!!!!!!!
            ...
            If (method.eq.2) then
               ...
               !       w(i)=w(i)+hamiltonianinteractvcvqtqg(j)*v(j_nz_vcvqtqg(j))
               !       w(j_nz_vcvqtqg(j))=w(j_nz_vcvqtqg(j))+hamiltonianinteractinst3t5(j)*v(i)
               !    end do
               ! end do

            endif

         else if (particle .eq. 2) then !!!!particle=2 corresponding to the quark system !!!!!!!!!!!!!!!!
            ...
            If(method.eq.2) then
               ...
               do i=dimtot1+1,dimtot1+2*dimtot2
                  do j=i_nz_vcvgtqq(i),i_nz_vcvgtqq(i+1)-1
                     w(dimtot1+i)=w(dimtot1+i)+hamiltonianinteractvcvgtqq(j)*v(dim+j_nz_vcvgtqq(j))
                     w(dim+j_nz_vcvgtqq(j))=w(dim+j_nz_vcvgtqq(j))+hamiltonianinteractinst3t5(j)*v(dimtot1+i)
                  end do
               end do
               ...
               do i=1,dimtot1
                  do j=i_nz_oge3t5(i),i_nz_oge3t5(i+1)-1
                     w(i)=w(i)+hamiltonianinteractogevalue3t5(j)*v(j_nz_oge3t5(j))
                     w(j_nz_oge3t5(j))=w(j_nz_oge3t5(j))+hamiltonianinteractinst3t5(j)*v(i)
                  end do
               end do

               do i=1,dimtot1
                  do j=i_nz_inst3t5(i),i_nz_inst3t5(i+1)-1
                     w(i)=w(i)+hamiltonianinteractinst3t5(j)*v(j_nz_inst3t5(j))
                     w(j_nz_inst3t5(j))=w(j_nz_inst3t5(j))+hamiltonianinteractinst3t5(j)*v(i)
                  end do
               end do
