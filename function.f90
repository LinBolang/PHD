...
double precision function longitrans(Nmax, Pplus, np1, mp1, np2, mp2, nk1, mk1, nk2, mk2, kp1half, kp2half, &
    & kk1half, kk2half)
  use numbers
    ...
    do N = 0, sumn
      do M = -summ, summ
        ...
        If(factorp .ge. 0 .and. factork .ge. 0)then
          ...
          meddlevalue = TMC(N, M, factorp, mp1+mp2-M, np1, mp1, np2, mp2, tandeltap)*TMC(N, M, factork, mk1+mk2-M, &
            & nk1, mk1, nk2, &
...
double precision function vertexspinorgtqq(Kt, n, b, mass1, mass2, kp1, kk1, kk2, sp1, sk1, sk2)
  ...
  if(sk1 .eq. 1 .and. sk2 .eq. -1) then
    vertexspinorgtqq = 2*Sqrt(dble(n+1))*(kk1half/kp1half*KroneckerDelta(sp1, 1)-kk2half/kp1half*KroneckerDelta(sp1, -1))&
      &*(-1)**(n)
  else if(sk1 .eq. -1 .and. sk2 .eq. 1) then
    vertexspinorgtqq = 2*Sqrt(dble(n+1))*(-kk2half/kp1half*KroneckerDelta(sp1, 1)+kk1half/kp1half*KroneckerDelta(sp1, -1))&
      &*(-1)**(n)
  else if(sk1 .eq. 1 .and. sk2 .eq. 1) then
...
double precision function instolap(Nmax, n, nprime, b, binst, kp1half, kp2half, kk1half, kk2half, Pplus)
  ...
  do capn = 0, Nmax-3
    ...
    if(nr .ge. 0) then
      ...
      instolap = instolap+TMC(capn, 0, nr, 0, n, 0, nprime, 0, 1.0D0)*b*(-1)**capn/Sqrt(Pi)*olaps(b, nr, 0, &
      ...
      do capn = 0, Nmax2-5
        ...
        If(n .ge. 0 .and. nprime .ge. 0) then
          ...
          Instantaneous3t5 = Instantaneous3t5+TMC(capn, mk2+mk3, nprime, 0, nk2, mk2, nk3, mk3, Sqrt(kk3half/kk2half)) &
            & *TMC(np1, mp1, n, 0, capn, mk2+mk3, nk1, mk1, Sqrt(kk1half/(kk2half+kk3half)))*instolap3t5(nmax2, n, &
            &nprime, b, &
